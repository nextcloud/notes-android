...
public class InteractiveNoteListWidgetConfigurationActivity extends LockedActivity {
    ...
    private String selectedItemId = MainActivity.ADAPTER_KEY_RECENT;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        binding.saveWidget.setOnClickListener(v -> saveWidget());
    ...
    private void applyNavigationItems(List<NavigationItem> navigationItems) {
        adapterCategories.setItems(navigationItems);

        navigationItems.stream()
                .filter(item -> item.id.equals(selectedItemId))
                .findFirst()
                .or(() -> navigationItems.stream()
                        .filter(item -> MainActivity.ADAPTER_KEY_RECENT.equals(item.id))
                        .findFirst())
    ...
    private void saveWidget() {
        ...
        applyNavigationMode(data, selectedItem);
    ...
    private void loadAccount() {
        executor.submit(() -> {
            ...
            final var storedData = repo.getNoteListWidgetData(appWidgetId);
            ...
            runOnUiThread(() -> {
                ...
                viewModel.getAdapterCategories(localAccount.getId()).observe(this, this::applyNavigationItems);
            });
        });
    }
    ...
    public void applyBrand(int color) {
        ...
        util.material.themeExtendedFAB(binding.saveWidget);
    }
}
